...
async def download_sparse_window(
    ...
    """Download only header/tail plus a targeted byte window from Telegram."""
    ...
    desired = max(WINDOW_BYTES, int(size * min(0.08, window_seconds / duration)))
    desired = min(MAX_WINDOW_BYTES, desired)
    ...
    path = root / f"remote_{message_id}_{center}_{desired}.bin"
