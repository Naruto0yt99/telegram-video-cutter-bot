...
from telegram_remote import download_sparse_window, get_telegram_video_info
...
async def _candidate_window(client, candidate, user_id, source_hint, width):
    ...
    return await download_sparse_window(
        client, chat, message_id, user_id, max(0.0, source_hint), window_seconds=width
    )
...
async def _probe_window(window_path, output_path, duration, start=0.0):
    try:
        await run_command(
            FFMPEG_BIN, "-y", "-ss", str(max(0.0, start)), "-i", str(window_path),
            "-t", str(max(1.0, duration)), "-map", "0:v:0?", "-map", "0:a:0?",
...
async def _try_candidate(client, candidate, segment, user_id, job_dir):
    ...
    if hint is None:
        # No source timestamp: use a bounded proportional search over the episode.
        # Each probe transfers only the sparse header/tail plus a small byte window.
        probe_ratios = [0.08, 0.24, 0.40, 0.56, 0.72, 0.88]
        for ratio in probe_ratios:
            source_hint = ratio * source_duration
            try:
                meta = await _candidate_window(
                    client, candidate, user_id, source_hint, max(12.0, duration + 8.0)
                )
                probe = job_dir / f"probe_{candidate['episode']}_{int(source_hint)}.mp4"
                ...
                if probe.exists() and probe.stat().st_size:
                    verified = await verify_candidate_window(
                        probe, segment, source_hint, source_hint + duration + 8.0
                    )
    ...
    hint = max(0.0, min(float(hint), source_duration))
    widths = [max(12.0, duration + 8.0), max(24.0, duration + 16.0)]
    for width in widths:
        try:
            meta = await _candidate_window(client, candidate, user_id, hint, width)
            ...
            if not probe.exists() or probe.stat().st_size == 0:
                meta["path"].unlink(missing_ok=True)
                continue
            verified = await verify_candidate_window(probe, segment, hint, hint + width)
            if verified.get("match") and verified.get("confidence", 0) >= 0.72:
                return meta, probe, verified
            meta["path"].unlink(missing_ok=True)
        except Exception as exc:
...
async def _process_segment(client, segment, index, user_id, job_dir):
    ...
    for candidate in candidates:
        ...
        clip = await make_clip(probe, local_start, local_end, name=f"find_{index:03d}")
        meta["path"].unlink(missing_ok=True)
        return {
            "index": index,
            "clip": clip,
            ...
            async with semaphore:
                if progress_message:
                    try:
                        ...
                        pass
                try:
                    return await _process_segment(telethon_client, segment, index, user_id, job_dir)
                except Exception as exc:
                    logger.exception("Scene %s failed: %s", index, exc)
                    return None

        results = await asyncio.gather(*(worker(i, segment) for i, segment in enumerate(segments, 1)))
