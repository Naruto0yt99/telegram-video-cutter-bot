...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "→ Telegram se sirf targeted source windows access honge\n"
        "→ local visual search likely locations ko narrow karega\n"
        "→ Gemini tiny candidates ko exact verify karega\n"
        "→ verified scenes parallel process honge; failed scenes skip honge\n"
        "→ sab verified scenes ko edit order me merge karega\n"
        "→ end me skipped/warnings ki full process report milegi\n\n"
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(nav_library_callback, pattern=r"^la:|^lr:|^ls:|^lb$"))
